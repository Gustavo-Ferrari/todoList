const todoApp = {
  ...
  addEventListeners() {
    document.querySelector('#addButton').addEventListener('click', () => this.handleAddTask());
    document.querySelector('#clearButton').addEventListener('click', () => this.clearList());
  ...
  createListItem(taskText) {
    ...
    const removeButton = this.createButton('Remove', this.removeTask.bind(this));
  ...
  updateLocalStorage() {
    const listItems = this.todoList.querySelectorAll('li');
    const tasks = Array.from(listItems).map(li => li.querySelector('p').textContent);
